    ...
    completedMatches.forEach(match => {
      ...
      if (homeTeam && awayTeam) {
        ...
        if (matchPoints && matchPoints.length > 0) {
          ...
          // Filter match points for this match
          const matchPointsRecords = matchPoints.filter(mp => mp.matchId === match.id && mp.hole === null);
          console.log(`Found ${matchPointsRecords.length} match points records for match ID: ${match.id}`);
