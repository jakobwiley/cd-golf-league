...
export default function SchedulePage({ 
  ...
  // Fetch data directly if not provided by server
  useEffect(() => {
    const fetchData = async () => {
      if (initialMatches.length > 0 && initialTeams.length > 0) {
      ...
      try {
        setLoading(true);
        console.log('Client-side fetching schedule data...');
        ...
        console.log(`Client received ${data.teams?.length || 0} teams and ${data.matches?.length || 0} matches`);
        
        if (!data.matches || !data.teams) {
          throw new Error('API response is missing matches or teams data');
        }
  ...
  useEffect(() => {
    if (matches && matches.length > 0) {
      ...
      const weekDates = matches.reduce((acc: Record<number, Date>, match) => {
    ...
    return (
      <div className="min-h-screen bg-[#030f0f] relative overflow-hidden">
        ...
        <div className="relative z-10 flex items-center justify-center min-h-screen">
          <div className="text-white text-xl font-orbitron">Loading schedule...</div>
        </div>
      </div>
    );
  }
  ...
  if (error) {
    return (
      <div className="min-h-screen bg-[#030f0f] relative overflow-hidden">
        ...
        <div className="relative z-10 flex items-center justify-center min-h-screen">
          <div className="text-red-500 text-xl font-orbitron max-w-md text-center">
            <p>Error loading schedule:</p>
            <p className="mt-2">{error}</p>
            <button 
              className="mt-4 px-4 py-2 bg-[#00df82]/20 border border-[#00df82]/50 rounded-lg text-white hover:bg-[#00df82]/30 transition-colors"
      ...
      <div className="relative z-10 mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="space-y-2">
          {weekNumbers.length > 0 ? (
            ...
            <div className="text-center py-10 text-white font-orbitron">
